...
%, though IANAL.  
A wide variety of licenses exist for molecular dynamics
...
languages impose constraints like types: that you can never add a
number and a string is the most basic example, but ML's functors
provide princpled ways of plugging in components with their
implementations completely hidden. 

Aggressive type checking avoids a subset of bugs which can arise due
to incorrectly written functions e.g. well publicised NASA problems
with a Mars orbiter
(http://www.cnn.com/TECH/space/9909/30/mars.metric.02/).  One example
is a pressure coupling bug in GROMACS~\cite{Hess2008},
...
Integration servers, VMs and cloud can make it easier; see
...
also push an C++ implementation of your algorithm to the WSSSPE
Cloud's section on this standard problem.
...
WSSSPE Cloud's database. Maybe, after some negotiation because your
angle on this standard problem is a bit novel, you add some of your
own tests to the database too. 

Pushing your code up activates the Cloud's Continuous Integration
...
The proposal above brings together almost all of the points we've
